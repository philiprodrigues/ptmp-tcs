...
/***Initialization of specific parameters for the DUNE 1x2x6 geometry/digitization.
...
Sorting a given vector of primitive characteristics (charge, time, channel) for an entire drift into independent vectors of hits into the desired windows.
***/
...
5) First channel (of APA) of biggest adjacency (ADD adjacency to get last channel)
6) First tick (of window) of biggest adjacency
7) Last tick (of window) of biggest adjacency
 ***/
vector<int> AdjacentSameWindowCountingPrimWithEverything(vector<unsigned int> channels, vector<unsigned int> adcs, vector<unsigned int> tots, vector<unsigned int> ticks){
  vector<int> max(5,0);
  int adj = 1;
  int tpc_1_sum = 0;
  int tpc_2_sum = 0;
  ...
  unsigned int first_channel = 0;
  unsigned int adj_first_channel = 0;
